package com.ankit.jpastreamer.service;
...
import com.ankit.jpastreamer.repository.EmployeeRepository;
...
public class EmployeeService {
  ...
  public List<Employee> getEmployeesByDept(String dept) {
    return jpaStreamer.stream(Employee.class).filter(Employee$.dept.equal(dept))
        .collect(Collectors.toList());
  }
  ...
  public List<Employee> getEmployeesByIds(List<Integer> ids) {
    return jpaStreamer.stream(Employee.class).filter(Employee$.id.in(ids))
        .collect(Collectors.toList());
  }
